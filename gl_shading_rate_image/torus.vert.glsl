...
void main()
{
  ...
  if (scene.fullShadingRateForGreenObjects == 1)
  {
    ...
    {
      // viewport 1 (mask 2) has a palette which maps all shading rate image values to full shading rate
      gl_ViewportMask[0] = 2;
    }
    else
    {
      // viewport 0 (mask 1) has the palette with the user defined shading rate
      gl_ViewportMask[0] = 1;
    }
  }
  else
